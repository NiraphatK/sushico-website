...
@section('js_before')
    ...
    <script>
        ...
        // ------- Client-side Search & Filters (ในหน้านี้เท่านั้น) -------
        (function() {
            ...
            const cards = Array.from(document.querySelectorAll('.js-user-card'));
            ...
            function match(itemSearch, itemRole, itemStatus, kw, fRole, fStatus) {
                if (kw && !itemSearch.includes(kw)) return false;
                if (fRole && fRole !== itemRole) return false;
            ...
            function apply() {
                ...
                const fRole = role.value.toLowerCase();
                ...
                rows.forEach(tr => {
                    const ok = match(tr.dataset.search, tr.dataset.role, tr.dataset.status, kw, fRole, fStatus);
                    tr.style.display = ok ? '' : 'none';
                    if (ok) shownTable++;
                });

                cards.forEach(card => {
                    const ok = match(card.dataset.search, card.dataset.role, card.dataset.status, kw, fRole,
                        fStatus);
                ...
                // info chip (เฉพาะฟิลเตอร์ฝั่ง client ของหน้านี้)
                const hasClientFilter = !!(kw || fRole || fStatus);
                clientInfo.classList.toggle('d-none', !hasClientFilter);
                if (hasClientFilter) {
                    const parts = [];
                    if (kw) parts.push(`q: "${kw}"`);
                    if (fRole) parts.push(`role: ${fRole}`);
